...
\EndFor
\EndFor
\State{Replace missing values with zero}
\end{algorithmic}
\end{algorithm}
...
\State{Compute the filtered probabilites of each state
...
\State{Find a state which has the maximum value in the filtered probabilites}
\EndFor
\end{algorithmic}
\end{algorithm}
...
Outputs from applying Markov switching models with each dataset are
shown here.
